...
- [ ] `computeVRFMsg` 출력이 `VCT_VRF ∥ chainId ∥ phash ∥ h` 포맷과 일치하는지 검증하는 단위 테스트
- [ ] `computeMiningSigMsgVCT` 출력이 `VCT_MINE ∥ sealHash ∥ ν` 포맷과 일치하는지 검증하는 단위 테스트
- [ ] `computePowSeedVCT` 출력이 `VCT_ECCPOW ∥ sealHash ∥ ν ∥ σ` 포맷과 일치하는지 검증하는 단위 테스트
- [ ] VRF 증명 생성·검증 왕복 테스트 (`VRFProve` → `VRFVerify`)
- [ ] `Address(PK_i) = coinbase` 검증: 일치하는 경우 통과, 불일치하는 경우 거부 테스트
- [ ] $y_i / 2^{|y|} < p_h$ 자격 임계값 판정 테스트 (경계값 포함)
- [ ] `ECRecover(σ_ν, m_ν) = a_i` 채굴 서명 검증 테스트
- [ ] ECCPoW hash vector가 VCT seed (`powSeedVCT`)와 레거시 seed (`powSeed`) 각각에서 올바르게 생성되는지 테스트
- [ ] VCTBlock 경계 테스트: VCTBlock-1은 레거시 규칙, VCTBlock은 WIP-6 규칙 적용
