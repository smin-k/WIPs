...
* [Specification / 명세](#specification--명세)
  ...
  - [Eligibility Threshold Adjustment](#eligibility-???-??)
* [Rationale](#rationale)
...
VCT는 32-byte VRF 출력 `beta` 전체를 big-endian unsigned 256-bit integer로 해석하고, 블록 헤더에 포함된 합의 임계값 `eligibilityThreshold`와 비교한다. 다음 조건을 만족하면 계정 $a_i$는 높이 $h$에 대해 **기본(즉시) VRF 자격**을 얻는다:

$$\mathrm{uint256}(\beta_i) < \texttt{eligibilityThreshold}_h$$

여기서 `eligibilityThreshold`는 $0 < P_h \le 2^{256}$ 범위의 uint256 값이다. 즉 기본 즉시 자격 확률은 $P_h / 2^{256}$이다. `eligibilityThreshold = 2^256`이면 모든 가능한 VRF 출력이 자격을 얻고, `eligibilityThreshold = 2^253`이면 기본 즉시 자격 확률은 $1/8$이다.
...
- `eligibilityThreshold`: 해당 블록의 기본 VRF 자격 임계값.

WIP-2 ECCPoW의 `codeword`와 `codeLength` 필드는 그대로 사용한다. 별도의 `vrfOutput` 필드는 두지 않으며, 검증자는 `vrfProof`에서 VRF output `beta`를 계산한다.

VCTBlock 이전 블록에서는 `vrfPublicKey`, `vrfProof`, `vrfSignature`, `eligibilityThreshold`를 사용하지 않는다.

### 점진적 타임아웃

고정 자격 임계값 `eligibilityThreshold`만을 사용하면, 블록 높이 $h$에서 모든 채굴 참여자가 동시에 부적격(ineligible)인 경우 체인이 영구적으로 정지할 수 있다. $N$명의 독립 채굴자가 있고 기본 확률이 $p_h = P_h/2^{256}$일 때 모든 채굴자가 기본 임계값에서 부적격일 확률은 $(1 - p_h)^N$이며, 채굴자 수가 적을수록 이 위험이 커진다. 점진적 타임아웃은 부모 블록 대비 경과 시간 $\Delta t$가 커질수록 자격 임계값을 자동으로 확대하여 활성성(liveness)을 보장하는 메커니즘이다.

**경과 블록 시간.** 높이 $h$의 후보 블록에 대해 원시 헤더 경과 시간 $\Delta t_\mathrm{raw}$와 유효 경과 시간 $\Delta t_\mathrm{eff}$를 정의한다:

$$\Delta t_\mathrm{raw} := \text{block.timestamp}_h - \text{block.timestamp}_{h-1}$$

$$\Delta t_\mathrm{eff} := \max(0, \Delta t_\mathrm{raw} - F)$$

$F$는 VCT 미래 타임스탬프 허용 상수이다. $\Delta t_\mathrm{raw}$는 블록 헤더의 타임스탬프를 사용하므로 모든 검증 노드가 동일하게 계산할 수 있고, $\Delta t_\mathrm{eff}$는 허용된 미래 타임스탬프 여유가 타임아웃 자격 확대로 이어지지 않도록 $F$초를 차감한 값이다. 점진적 타임아웃은 $\Delta t_\mathrm{raw}$가 아니라 $\Delta t_\mathrm{eff}$를 기준으로 판정한다.

**시간 의존 자격 임계값.** 파라미터 $t_s$, $t_e$ ($t_e > t_s \geq 0$)를 정의한다:

- $t_s$: 타임아웃 시작 임계값(초). $\Delta t_\mathrm{eff} < t_s$이면 자격 임계값이 헤더의 기본값 `eligibilityThreshold`로 유지된다.
- $t_e$: 타임아웃 종료 임계값(초). $\Delta t_\mathrm{eff} \geq t_e$이면 모든 채굴자가 자격을 얻는다.
- $\tau_h$: 지수 스케일 상수. $\tau_h = \dfrac{t_e - t_s}{\ln(2^{256} / P_h)}$, where $P_h$ is the block header's `eligibilityThreshold`.

시간 의존 자격 임계값 `P(Δt_eff)`는 다음과 같이 정의된다:

$$P(\Delta t_\mathrm{eff}) = \begin{cases} P_h & \text{if } \Delta t_\mathrm{eff} < t_s \\ \left\lfloor P_h \cdot \exp\!\left(\dfrac{\Delta t_\mathrm{eff} - t_s}{\tau_h}\right) \right\rfloor & \text{if } t_s \leq \Delta t_\mathrm{eff} < t_e \\ 2^{256} & \text{if } \Delta t_\mathrm{eff} \geq t_e \end{cases}$$

이 정의에서 $P(t_s) = P_h$이고 $P(t_e) = 2^{256}$이므로, 타임아웃 종료 시점에는 모든 32-byte VRF 출력이 자격을 얻는다.

**VRF 임계값.** 자격 조건은 다음과 같이 uint256 비교로 판정한다:

$$\mathrm{uint256}(\beta_i) < P(\Delta t_\mathrm{eff})$$

`P(Δt_eff) = 2^256`인 경우에는 모든 가능한 VRF 출력이 조건을 만족하는 것으로 처리한다.

**채굴자의 제출 대기 시간.** $\mathrm{uint256}(\beta_i) \geq P_h$인 채굴자(기본 임계값에서 부적격)는 $P(\Delta t_\mathrm{eff}) > \mathrm{uint256}(\beta_i)$가 되는 최소 유효 경과 시간 $t_\text{submit}(\beta_i)$까지 기다린 후 블록을 제출한다:

$$t_\text{submit}(\beta_i) = t_s + \tau_h \cdot \ln\!\left(\frac{\mathrm{uint256}(\beta_i) + 1}{P_h}\right)$$

즉, 해당 채굴자는 $\text{block.timestamp}_{h-1} + t_\text{submit}(\beta_i) + F$ 이상의 원시 헤더 타임스탬프를 사용한 블록만 제출할 수 있다. $\mathrm{uint256}(\beta_i) < P_h$인 채굴자는 대기 없이($t_\text{submit} = 0$) 즉시 채굴을 시작할 수 있다.

**결정론적 검증.** 검증 노드는 제출된 블록의 $\text{block.timestamp}_h$와 부모 블록의 $\text{block.timestamp}_{h-1}$로 $\Delta t_\mathrm{raw}$를 계산하고, $\Delta t_\mathrm{eff} = \max(0, \Delta t_\mathrm{raw} - F)$를 적용한 뒤 `P(Δt_eff)`를 재현하여 $\mathrm{uint256}(\beta_i) < P(\Delta t_\mathrm{eff})$ 조건을 판정한다. 노드의 로컬 시간에 의존하지 않으므로, 동일한 블록 헤더에 대해 네트워크 내 모든 노드가 동일한 판정을 내린다.

**기본 파라미터.** 목표 기본 임계값은 `P_base = 2^253`($p^\star = 1/8$), $t_s = 15$초로 둔다. 타임아웃 종료값 $t_e$는 평균 블록 발견 시간을 10초로 보는 지수분포의 99.9% 분위수로 정한다:

$$t_e = \left\lceil -10 \cdot \ln(1 - 0.999) \right\rceil = \left\lceil 10 \cdot \ln(1000) \right\rceil = 70\text{ seconds}.$$

따라서 $\Delta t_\mathrm{eff}=70$초에서 모든 VRF 출력이 자격을 얻는다. $F=5$초를 적용하므로 원시 헤더 경과 시간 기준으로는 $\Delta t_\mathrm{raw}=75$초에서 완전 개방된다. 하드포크 직후 bootstrap 구간에서는 `eligibilityThreshold = 2^256`으로 시작하여 활성성을 보장하고, 이후 deterministic controller를 통해 `P_base`까지 낮춘다.
...
VCTBlock 이후 클라이언트는 미래 타임스탬프 허용 상수 `allowedFutureBlockTimeSeconds`를 5초로 줄인다. 현재 시각보다 5초를 초과하여 미래인 블록은 future block으로 처리한다. 점진적 타임아웃의 자격 임계값은 부모 블록과 후보 블록의 헤더 타임스탬프 차이에서 미래 허용 상수 $F=5$초를 차감한 $\Delta t_\mathrm{eff}$로 계산한다:
...
VCTBlock 이후 `sealHash`는 기존 ECCPoW header commitment에 VCT 자격 증명 필드를 추가로 커밋한다. 추가되는 필드는 `vrfPublicKey`, `vrfProof`, `eligibilityThreshold`이다. 반대로 nonce $\nu$, mining signature $\sigma_\nu$, Codeword, CodeLength는 ECCPoW 탐색 과정에서 결정되므로 `sealHash`에서 제외한다.
...
VCTBlock 이후 블록 생성 절차는 기존 ECCPoW 채굴 루프 앞에 세 가지 단계를 추가한다. 첫째, 채굴 계정은 부모 상태에서 $S_0$ 이상의 잔액을 가져야 한다. 둘째, 부모 블록 해시와 목표 높이에 대해 ECVRF를 평가하고, VRF 출력이 현재 eligibility 임계값 또는 점진적 타임아웃 임계값을 만족해야 한다. 셋째, 블록 헤더에는 VCT 자격 증명 필드 `vrfPublicKey`, `vrfProof`, `eligibilityThreshold`가 포함되어야 한다.
...
VCTBlock 이후 수신 노드는 기존 ECCPoW 검증에 다음 검증을 추가한다. 먼저 블록에 포함된 공개키가 coinbase 주소와 일치하는지 확인하고, 부모 상태에서 해당 계정의 잔액이 $S_0$ 이상인지 확인한다. 다음으로 부모 블록 해시와 블록 높이에 대한 ECVRF 증명과 출력이 유효한지 검증하고, $\mathrm{uint256}(\beta_i) < P(\Delta t)$ 조건으로 eligibility 자격을 판정한다. 마지막으로 고정된 `sealHash`에 대해 논스별 채굴 서명 $\sigma_\nu$가 coinbase 계정 키로 생성되었는지 확인하고, 이 서명을 포함한 `powSeed`로 기존 ECCPoW hash vector와 LDPC codeword 검증을 수행한다.

### Eligibility Threshold Adjustment

VCTBlock 이후 블록 헤더는 `eligibilityThreshold`를 포함한다. 목표 임계값은 `P_base = 2^253`($p^\star = 1/8$)이며, 하드포크 직후에는 `eligibilityThreshold = 2^256`에서 시작하여 deterministic bootstrap controller가 목표값까지 낮춘다. 블록 시간이 지연되는 경우에는 [점진적 타임아웃](#점진적-타임아웃)의 시간 의존 임계값 `P(Δt_eff)`가 해당 블록의 활성성을 보완한다.

여기서 `P_base`는 계속 움직이는 값이 아니라 `eligibilityThreshold`가 수렴해야 하는 목표값이다. 평균 블록 생성 시간은 ECCPoW difficulty가 조정하고, VRF 임계값은 하드포크 직후 bootstrap 구간과 timeout 구간에서만 활성성 보완을 위해 조정된다.

자격 채굴자 수는 자연수이므로 eligibility probability는 ECCPoW difficulty처럼 정밀한 난이도 대체물이 되기 어렵다. 따라서 `eligibilityThreshold`는 admission-control 파라미터로 사용하고, 평균 블록 시간 조정은 ECCPoW difficulty에 맡긴다.
...
**점진적 타임아웃 대 고정 임계값.** 고정 임계값 `P_base`만 사용하면 $N$명의 채굴자가 모두 동시에 부적격일 수 있으며, 이 경우 체인이 영구 정지한다. 채굴자 수를 늘리면 이 확률이 지수적으로 감소하지만, 초기 네트워크에서는 $N$이 작아 위험이 높다. 점진적 타임아웃은 유효 경과 시간 $\Delta t_\mathrm{eff}$에 따라 바이트 임계값 `P(Δt_eff)`를 확대하여, 충분한 시간이 지나면 반드시 누군가가 블록을 생성할 수 있도록 활성성을 보장한다. 이 메커니즘이 고정 타임아웃(예: Δt_eff ≥ T_hard이면 즉시 모든 채굴자 자격)보다 점진적 확대를 택한 이유는, 급격한 임계값 전환이 블록 스탬핑 공격이나 타임스탬프 조작 인센티브를 증가시키기 때문이다. 미래 타임스탬프 허용분 $F$는 $\Delta t_\mathrm{eff}$ 계산에서 차감하므로, 허용된 미래 timestamp가 timeout 자격을 앞당기지 않는다.

**VRF 임계값 조정 범위.** `eligibilityThreshold` 조정은 ECCPoW difficulty의 대체물이 아니다. 자격 계정 수는 자연수이므로 임계값을 아주 미세하게 바꿔도 실제 자격자 수가 달라지지 않을 수 있고, 블록 시간은 이미 ECCPoW difficulty가 조정한다. 따라서 VRF 임계값 조정은 하드포크 직후 bootstrap과 점진적 타임아웃에 한정한다.
