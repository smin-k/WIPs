...
$$\texttt{sealHash} := \mathrm{Keccak256}\!\left(\texttt{VCT}\_\texttt{SEAL} \| \texttt{chainId} \| \mathrm{RLP}\!\left(\mathsf{phash}_{h-1},\, h,\, a_i,\, PK_i,\, y_i,\, \pi_i,\, \texttt{timestamp},\, \ldots,\, \texttt{CodeLength}\right)\right).$$
...
$$m_\nu := \mathrm{Keccak256}(\texttt{VCT}\_\texttt{MINE} \| \texttt{chainId} \| \texttt{sealHash} \| \nu),$$

$$\sigma_\nu := \mathrm{Sign}_{sk_i}(m_\nu).$$

그 다음 ECCPoW hash-vector seed를 다음과 같이 정의한다:

$$\texttt{powSeed}_\nu := \mathrm{Keccak256}(\texttt{VCT}\_\texttt{ECCPOW} \| \texttt{chainId} \| \texttt{sealHash} \| \nu \| \sigma_\nu).$$
...
$$\sigma_\nu = \mathrm{Sign}_{sk_i}(\mathrm{Keccak256}(\texttt{VCT}\_\texttt{MINE} \| \texttt{chainId} \| \texttt{sealHash} \| \nu)).$$
