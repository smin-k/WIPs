...
VCT 프로토콜은 다른 위치를 점한다. 스테이킹 컨트랙트나 검증자 레지스트리를 도입하지 않고, 부모 상태의 계정 잔액만을 자격 기준으로 사용한다. VRF 자격 임계값은 각 블록에서 ECCPoW를 수행할 수 있는 계정 수를 제어하며, 최종 블록 경쟁은 여전히 자격을 갖춘 계정들 간의 ECCPoW 연산 경쟁으로 결정된다. Rokis v1.0에서는 기본 VRF 자격 임계값을 고정하고, 평균 블록 생성 시간 조정은 기존 ECCPoW difficulty가 담당한다. VRF 임계값의 동적 조정은 difficulty 조정과 밀접하게 결합되어야 하므로 후속 WIP의 범위로 남긴다.
...
**구현 파라미터.** 클라이언트 구현은 `VctConfig.MinEligibleBalance`를 $S_0$ 값으로 사용한다. 테스트넷 실험을 위해 `S0ForkBlock`과 `S0ForkBalance`를 둘 수 있으며, 이 경우 블록 번호가 `S0ForkBlock` 이상이면 $S_0$는 `S0ForkBalance`로 변경된다. 이 기능은 자격 임계값 변경 실험을 위한 구현 파라미터이며, 메인넷 활성화 시에는 하드포크 설정에 포함되어 모든 노드가 동일한 값을 사용해야 한다.
...
**한계.** $\ell = 1$은 장기 lock-up이나 bonded staking을 의미하지 않는다. 계정은 부모 상태에서 $S_0$ 이상 잔액을 보유하기만 하면 다음 블록의 자격 조건을 만족하므로, 단기 차입이나 일시적 잔액 이동을 통해 자격을 얻는 전략은 가능하다. WIP-6 v1.0은 이를 명시적인 설계 trade-off로 받아들인다. $S_0$는 지분 지속 기간을 강제하는 장치가 아니라 Sybil 계정 생성과 ECCPoW 참여에 필요한 최소 유동성 비용을 부과하는 장치다. 더 강한 stake-duration 요구, lock-up, unbonding period, 또는 최근 여러 블록의 최소/평균 잔액 조건은 별도 상태 관리와 sync 복잡도를 요구하므로 후속 WIP에서 다룬다.
...
**WorldLand secp256k1 ECVRF ciphersuite.** Rokis profile에서 사용하는 VRF ciphersuite는 다음 바이트 수준 규칙으로 고정된다. 이 규칙은 RFC 9381의 표준 ciphersuite가 아니며, WorldLand 합의 규칙의 일부이다. 다른 클라이언트 구현은 아래 절차와 테스트 벡터를 그대로 재현해야 한다.
...
**Ciphersuite test vector.** 다음 벡터는 합의 구현이 동일한 ciphersuite를 사용하는지 확인하기 위한 최소 테스트 벡터이다.
...
**키 등록과 채굴 unlock.** 참조 구현은 coinbase 주소에서 VRF 개인 키를 파생하지 않는다. 주소는 공개 정보이므로 `Hash(coinbase)`와 같은 방식으로 VRF 개인 키를 만들면 누구나 동일한 키를 계산할 수 있고, 자격 증명을 위조할 수 있다. 따라서 VCTBlock 이후 채굴자는 실제 계정 개인 키를 unlock해야 하며, 클라이언트는 unlock된 계정 키를 VCT 엔진에 등록한 경우에만 VRF 증명과 논스별 채굴 서명을 생성한다. VCTBlock 이전 pre-VCT 구간은 기존 ECCPoW 호환성을 위해 VRF 키 등록과 계정 unlock을 요구하지 않는다.
...
Rokis v1.0은 VRF 출력 전체를 실수로 정규화하여 비교하지 않고, 32-byte VRF 출력 `beta`의 첫 번째 바이트를 합의 임계값과 비교한다. 즉, 기본 자격 임계값은 8-bit 정수 `P_base`로 표현된다. 다음 조건을 만족하면 계정 $a_i$는 높이 $h$에 대해 **기본(즉시) VRF 자격**을 얻는다:

$$\beta_i[0] < P_{\mathrm{base}}$$

여기서 `P_base`는 기본 VRF 자격 확률(base eligibility probability)을 바이트 단위로 나타낸 값이다. 예를 들어 `P_base = 0xA0`이면 기본 즉시 자격 확률은 $160/256 = 62.5\%$이다. 실제 채굴 및 검증에서 사용되는 시간 의존 임계값 `P(Δt)`는 [점진적 타임아웃](#점진적-타임아웃) 절에서 정의한다. $\Delta t$가 충분히 크면 임계값이 `0x100`에 도달한 것으로 간주되어 모든 채굴자가 자격을 얻는다. 이 첫 바이트 비교 규칙은 consensus-critical하며, 다른 클라이언트가 VRF 출력 전체를 정수로 해석하여 비교해서는 안 된다.

이 구조는 세 가지 핵심 특성을 가진다:

1. **지역성(Locality).** VRF는 $sk_i$와 공개된 $\mathsf{phash}_{h-1}$만으로 계정 $a_i$의 운영자 혼자 계산할 수 있다. 다른 노드와의 통신이 필요하지 않다.
2. **의사난수성(Pseudorandomness).** $sk_i$를 모르는 당사자에게 VRF 출력은 증명이 공개되기 전까지 난수와 계산적으로 구분할 수 없다. 입력에 $\mathsf{phash}_{h-1} \| h$를 포함함으로써 자격이 현재 체인 상태에 바인딩되고 유리한 입력을 자유롭게 선택하는 것이 제한된다. 단, parent hash grinding은 잔여 위험으로 남는다: 공격자가 서로 다른 $\mathsf{phash}_{h-1}$으로 VRF를 여러 번 시도하여 유리한 출력을 선택할 수 있다. 그러나 WIP-6에서 이 grinding은 무비용이 아니다 — 각 대안 parent hash는 유효한 ECCPoW 블록에 대응해야 하므로, 공격자는 추가 VRF 시도마다 실제 PoW 비용과 블록 보류(withholding) 위험을 감수해야 한다. WIP-6은 이를 주요 공격 벡터가 아닌 한정된 잔여 위험(bounded residual risk)으로 간주한다.
...
고정 자격 임계값 `P_base`만을 사용하면, 블록 높이 $h$에서 모든 채굴 참여자가 동시에 부적격(ineligible)인 경우 체인이 영구적으로 정지할 수 있다. $N$명의 독립 채굴자가 있을 때 모든 채굴자가 기본 임계값에서 부적격일 확률은 $(1 - P_{\mathrm{base}}/256)^N$이며, 채굴자 수가 적을수록 이 위험이 커진다. 점진적 타임아웃은 부모 블록 대비 경과 시간 $\Delta t$가 커질수록 자격 임계값을 자동으로 확대하여 활성성(liveness)을 보장하는 메커니즘이다.
...
- $t_s$: 타임아웃 시작 임계값(초). $\Delta t < t_s$이면 자격 임계값이 기본값 `P_base`로 유지된다.
- $t_e$: 타임아웃 종료 임계값(초). $\Delta t \geq t_e$이면 모든 채굴자가 자격을 얻는다.
- $\tau$: 지수 스케일 상수. $\tau = \dfrac{t_e - t_s}{\ln(256 / P_{\mathrm{base}})}$

시간 의존 자격 임계값 `P(Δt)`는 다음과 같이 정의된다:

$$P(\Delta t) = \begin{cases} P_{\mathrm{base}} & \text{if } \Delta t < t_s \\ \left\lfloor P_{\mathrm{base}} \cdot \exp\!\left(\dfrac{\Delta t - t_s}{\tau}\right) \right\rfloor & \text{if } t_s \leq \Delta t < t_e \\ 256 & \text{if } \Delta t \geq t_e \end{cases}$$

이 정의에서 $P(t_s) = P_{\mathrm{base}}$이고 $P(t_e) = 256$이므로, 타임아웃 종료 시점에는 모든 첫 바이트 값 `0x00..0xff`가 자격을 얻는다.

**VRF 임계값.** 자격 조건은 다음과 같이 첫 바이트 정수 비교로 판정한다:

$$\beta_i[0] < P(\Delta t)$$

`P(Δt) = 256`인 경우에는 모든 가능한 첫 바이트가 조건을 만족하는 것으로 처리한다.

**채굴자의 제출 대기 시간.** $\beta_i[0] \geq P_{\mathrm{base}}$인 채굴자(기본 임계값에서 부적격)는 $P(\Delta t) > \beta_i[0]$이 되는 최소 경과 시간 $t_\text{submit}(\beta_i[0])$까지 기다린 후 블록을 제출한다:

$$t_\text{submit}(\beta_i[0]) = t_s + \tau \cdot \ln\!\left(\frac{\beta_i[0] + 1}{P_{\mathrm{base}}}\right)$$

즉, 해당 채굴자는 $\text{block.timestamp}_{h-1} + t_\text{submit}(\beta_i[0])$ 이상의 타임스탬프를 사용한 블록만 제출할 수 있다. $\beta_i[0] < P_{\mathrm{base}}$인 채굴자는 대기 없이($t_\text{submit} = 0$) 즉시 채굴을 시작할 수 있다.

**결정론적 검증.** 검증 노드는 제출된 블록의 $\text{block.timestamp}_h$와 부모 블록의 $\text{block.timestamp}_{h-1}$로 $\Delta t$를 계산하고, `P(Δt)`를 재현하여 $\beta_i[0] < P(\Delta t)$ 조건을 판정한다. 노드의 로컬 시간에 의존하지 않으므로, 동일한 블록 헤더에 대해 네트워크 내 모든 노드가 동일한 판정을 내린다.

**Rokis v1.0 파라미터.** Rokis v1.0에서는 `P_base = 0xA0`, $t_s = 15$초, $t_e = 60$초를 합의 상수로 고정한다. `P_base`의 동적 조정은 본 WIP의 활성화 범위에 포함하지 않으며, 반복 timeout 빈도와 ECCPoW difficulty를 함께 고려하는 후속 WIP에서 다룬다.

**타임스탬프 조작 완화.** 블록 헤더의 타임스탬프 $\Delta t$는 채굴자가 직접 설정하므로, 채굴자가 $\Delta t$를 인위적으로 크게 설정하여 자격 임계값 확대의 이점을 얻을 수 있다. 이를 완화하기 위해 검증 노드는 실제 자격 판정에 원시 $\Delta t$ 대신 유효 경과 시간 $\Delta t_\text{eff}$를 사용한다:

$$\Delta t_\text{eff} := \max(0,\; \Delta t - F)$$

여기서 $F$는 합의 고정(consensus-fixed) 미래 타임스탬프 허용 상수(future tolerance constant)이다. 채굴자가 타임스탬프를 최대 $F$초 앞당겨도 $\Delta t_\text{eff}$에는 영향을 주지 않으므로, $F$ 이하의 타임스탬프 조작은 타임아웃 자격 확대로 직결되지 않는다.

$\Delta t_\text{eff}$를 기준으로 자격 확인 시, 채굴자가 실제로 블록을 제출하는 원시 타임스탬프는 다음 조건을 만족해야 한다:

$$\text{block.timestamp}_h \geq \text{block.timestamp}_{h-1} + t_\text{submit}(\beta_i[0]) + F$$

즉, 대기 시간 $t_\text{submit}(\beta_i[0])$는 유효 경과 시간 기준으로 계산하고, 거기에 $F$를 더한 원시 타임스탬프를 사용한다.

**한계.** 이 완화는 타임스탬프 조작의 비용을 높이는 것이지 완전히 제거하지는 않는다. 합의에서 타임스탬프를 입력으로 사용하는 한 완전한 제거는 불가능하다. $F$를 초과하는 타임스탬프 조작은 여전히 $\Delta t_\text{eff}$를 증가시킬 수 있으나, P2P 레이어의 미래 블록 거부 규칙(미래 $\leq F$초인 블록만 전파)이 실질적인 추가 방어선 역할을 한다.
...
5.  if β_i[0] < P_base then               ▷ 즉시 자격: 기본 임계값 이내
        t_h ← now()                        ▷ 현재 시간을 블록 타임스탬프로 사용
    else                                   ▷ 대기 자격: 타임아웃 이후 제출 가능
        t_h ← t_{h-1} + t_s + τ · ln((β_i[0] + 1) / P_base)
              ▷ τ = (t_e - t_s) / ln(256 / P_base)
        Wait until local_time ≥ t_h (or peer block h arrives — then abort)
    end if
    ▷ 이 시점에서 Δt = t_h - t_{h-1}이면 β_i[0] < P(Δt)가 보장됨
...
7.  Require β_i[0] < P(Δt)
    ▷ 고정 P_base 대신 시간 의존 바이트 임계값 사용; P(Δt)는 점진적 타임아웃 절 참조
...
VCTBlock 이전의 Seoul 구간에서는 기존 ECCPoW 보상 정책을 그대로 사용한다. 즉 pre-VCT 블록은 레거시 WorldLand 보상 스케줄에 따라 기본 보상 4 WL 및 기존 halving/maturity 규칙을 적용하며, 트레저리 분배는 수행하지 않는다. 이 구간은 기존 ECCPoW 노드와 동일한 state transition을 유지해야 하므로 보상 공식도 레거시와 일치해야 한다.

VCTBlock 이후 Rokis/VCT 구간에서는 블록 기본 보상을 20 WL로 설정한다. 블록 생성자 보상과 uncle 보상을 합산한 총 보상 중 20%는 프로토콜 트레저리 주소로 지급하고, 나머지 80%는 블록 생성자에게 지급한다.

현재 참조 구현의 트레저리 주소는 다음과 같다.

```text
0x4C7dE6771DC602176b25fD4E1ae5550A3eAa06dF
```

보상 정책은 합의 상태 전이에 포함되므로 모든 클라이언트가 동일한 VCTBlock 경계, 보상 금액, 트레저리 비율, 트레저리 주소를 사용해야 한다. 특히 하드포크 경계에서 pre-VCT 블록에 VCT 보상 정책을 적용하거나, VCT 블록에 레거시 보상 정책을 적용하면 state root가 달라져 네트워크가 분리된다.

### p 조정

Rokis v1.0은 동적 `P_base` 조정을 도입하지 않는다. 기본 sortition 임계값은 `P_base = 0xA0`으로 고정되며, 블록 시간이 지연되는 경우에는 [점진적 타임아웃](#점진적-타임아웃)의 시간 의존 임계값 `P(Δt)`가 해당 블록의 활성성을 보완한다.
...
또한 자격 채굴자 수는 자연수이므로 `P_base`를 미세하게 조정해도 실제 eligible proposer 수가 매끄럽게 변하지 않을 수 있다. 특히 초기 테스트넷처럼 자격 계정 수가 작을 때는 sortition probability가 정밀한 난이도 대체물이 될 수 없다. 따라서 Rokis v1.0에서는 `P_base`를 고정하고, ECCPoW difficulty가 평균 블록 시간을 조정하며, progressive timeout은 즉시 자격자가 없을 때의 liveness fallback으로만 사용한다.

반복적인 timeout이 장기간 발생하는 경우에는 `P_base`를 조정할 필요가 있을 수 있다. 그러나 이 기능은 timeout frequency, stale/uncle rate, ECCPoW difficulty 조정, 최소/최대 sortition threshold를 함께 고려하는 별도 joint controller로 설계되어야 한다. 이러한 adaptive probability 조정은 본 WIP의 활성화 범위가 아니라 후속 WIP의 대상으로 남긴다.

## Rationale

**잔액 기반 자격 조건.** 별도 스테이킹 컨트랙트나 검증자 레지스트리 없이 부모 블록 상태의 계정 잔액만으로 자격을 확인하는 방식을 선택한 이유는, 등록 트랜잭션·lock-up·슬래싱 메커니즘 없이 Sybil 공격 비용을 부과하면서 v1.0 설계 복잡성을 최소화하기 위해서다. $S_0$는 지분 비례 영향력이 아니라 ECCPoW 참여의 최소 진입 비용으로 설계된다. 지분 크기에 비례한 선출 확률을 부여하면 자본 집중에 따른 중앙화 압력이 생기므로, VCT는 임계값을 충족한 모든 계정에 동일한 확률 $p$를 부여한다.

**계정 키 재사용.** 별도의 VRF 공개 키 등록 절차를 제거하고 기존 Ethereum 계정 키 쌍 $(sk_i, PK_i)$를 ECVRF 입력 키로 재사용한다. 이는 키 등록 트랜잭션, 계정-합의키 바인딩 검증, 별도 키 관리 오버헤드를 피하기 위해서이며, DID 없는 v1.0 설계에서 구현과 사용자 경험을 단순화한다. 블록 헤더에 $PK_i$를 포함하고 $\mathrm{Address}(PK_i) = a_i$를 확인함으로써, 블록 생성자가 coinbase 주소에 대응하는 개인 키를 실제로 보유하고 있음을 VRF 증명과 채굴 서명이 일관되게 증명한다.
...
**Nakamoto 포크 선택.** VRF 자격 임계값 `P_base`는 블록당 기대 채굴 시도자 수를 제어하지만 체인 작업량 계산에는 포함되지 않는다. 최종 포크 선택은 누적 ECCPoW 작업량 기준 최장 체인 규칙을 따른다. 이는 VRF 자격 선발을 BFT 투표나 리더 선출로 전환하지 않고 기존 Nakamoto 합의의 보안 보장을 그대로 유지하기 위해서다. BFT-style 위원회 선출을 도입하면 liveness 분석과 네트워크 가정이 복잡해지므로, v1.0은 잔액 기반 사전 자격 확인을 Nakamoto ECCPoW 경쟁 위에 얹는 최소 설계를 채택한다.

**점진적 타임아웃 대 고정 임계값.** 고정 임계값 `P_base`만 사용하면 $N$명의 채굴자가 모두 동시에 부적격일 수 있으며, 이 경우 체인이 영구 정지한다. 채굴자 수를 늘리면 이 확률이 지수적으로 감소하지만, 초기 네트워크에서는 $N$이 작아 위험이 높다. 점진적 타임아웃은 경과 시간 $\Delta t$에 따라 바이트 임계값 `P(Δt)`를 확대하여, 충분한 시간이 지나면 반드시 누군가가 블록을 생성할 수 있도록 활성성을 보장한다. 이 메커니즘이 고정 타임아웃(예: Δt ≥ T_hard이면 즉시 모든 채굴자 자격)보다 점진적 확대를 택한 이유는, 급격한 임계값 전환이 블록 스탬핑 공격이나 타임스탬프 조작 인센티브를 증가시키기 때문이다. 단, 타임스탬프 조작에 의한 미약한 그라인딩 여지는 v1.0의 명시적 한계로 남는다.

**VRF 임계값 조정 범위.** 자격 확률을 Bitcoin difficulty처럼 실수 범위에서 연속적으로 조정하면 두 가지 문제가 생긴다. 첫째, 자격 계정 수는 자연수이므로 임계값을 아주 미세하게 바꿔도 실제 자격자 수가 달라지지 않을 수 있다. 둘째, 블록 시간을 이미 ECCPoW difficulty가 조정하고 있으므로, VRF 임계값도 블록 시간 신호로 조정하면 두 제어 루프가 간섭한다. 따라서 Rokis v1.0은 `P_base = 0xA0`을 고정한다. 반복 timeout에 대응하는 adaptive threshold는 필요할 수 있으나, 이는 timeout frequency, stale/uncle rate, ECCPoW difficulty, 최소/최대 임계값을 함께 고려하는 별도 joint controller로 설계해야 하며 본 WIP에서는 활성화하지 않는다.
...
점진적 타임아웃은 다음 방식으로 체인 정지 문제를 해결한다. 모든 $N$명의 채굴자가 기본 임계값에서 부적격($\beta_i[0] \geq P_{\mathrm{base}}$)인 상황에서도, 경과 시간 $\Delta t$가 $t_e$에 도달하면 `P(Δt) = 256`이 되어 모든 채굴자가 자격을 얻는다. 따라서 체인은 최대 $t_e$초 지연 후 반드시 다음 블록이 생성된다.

$\beta_i[0] < P_{\mathrm{base}}$인 즉시 자격 채굴자가 한 명이라도 존재하면, 해당 채굴자가 정상 블록을 생성하여 타임아웃 없이 체인이 진행된다. $t_s$를 목표 블록 간격 $\Delta_\text{target}$과 동일하게 설정하면, 네트워크가 정상 동작할 때 타임아웃이 거의 발생하지 않는다.

**타임스탬프 조작.** 채굴자가 자신의 VRF 첫 바이트에 유리한 블록 타임스탬프를 선택하여 조기에 블록을 제출하려 할 수 있다. 그러나 $\Delta t$가 줄어들면 `P(Δt)`도 줄어들므로, 타임스탬프를 과거로 당기면 오히려 자격 임계값이 낮아져 유리하지 않다. 반대로 미래 타임스탬프를 사용하면 더 높은 임계값을 얻을 수 있지만, Ethereum 계열 클라이언트는 미래 타임스탬프 블록을 일정 허용 오차(tolerance) 이상이면 거부하므로 실질적 이득이 제한된다. 또한 VRF 출력은 VRF 입력(phash, 높이)에 의해 이미 결정되므로, 채굴자가 출력 자체를 선택할 수 없다.

**공정한 대기.** 첫 바이트가 낮은 채굴자(VRF에서 유리한 출력)가 먼저 블록을 제출하고, 첫 바이트가 높은 채굴자는 더 오래 기다린다. 이는 VRF 출력에 비례한 대기 순서로, 동일 블록에 여러 채굴자가 자격을 얻는 경우 ECCPoW 경쟁으로 최종 블록이 결정된다는 점에서 기존 Nakamoto 방식의 보안 보장을 유지한다.
...
`insertChain`의 S₀ 검증(`VerifyProposerEligibility`)은 부모 상태(`parentState`)가 필요하다. snap sync 중에는 부모 상태가 없을 수 있으므로, 다음 설계 결정을 명시한다.

**설계 결정: 부모 상태 없음 → S₀ 검증 건너뜀.**

부모 상태가 없는 경우 S₀ 검증을 건너뛰고 블록을 수락한다. 이 결정의 근거:

1. ECCPoW 유효성 검증은 상태 독립적이며 항상 적용된다.
2. VRF 증명 검증도 항상 적용된다.
3. 공격자가 S₀ 미충족 계정으로 가짜 체인을 만들려면 실제 ECCPoW 해시파워로 정직한 체인보다 긴 체인을 구성해야 한다. S₀ 게이트 우회만으로 얻을 수 있는 이득은 없다.

snap sync pivot 이후 블록에서는 부모 상태가 항상 존재하므로 S₀ 검증이 정상 적용된다. pivot 이전 구간은 건너뜀으로 처리되며, 이는 위의 이유로 안전하다.
...
**pre-VCT 레거시 호환성.** VCT 엔진을 탑재한 클라이언트라도 VCTBlock 이전 블록은 기존 ECCPoW와 동일하게 처리해야 한다. pre-VCT 블록은 `VCT_SEAL` 도메인 프리픽스를 포함하지 않는 레거시 `sealHash`를 사용하고, ECCPoW hash-vector seed도 기존 `sealHash ∥ nonce` 입력을 사용한다. 이 구간에서는 VRF sortition, VRF proof/public key, 논스별 채굴 서명, $S_0$ 잔액 검증, VCT 트레저리 보상을 모두 적용하지 않는다. 이렇게 해야 기존 ECCPoW 노드가 생성한 pre-VCT 블록과 VCT 엔진이 생성한 pre-VCT 블록의 `MixDigest` 및 state root가 일치한다.
...
WIP-6은 기존 ECCPoW 클라이언트 대비 다음 여덟 가지 변경을 요구한다.
...
6. **VCT 보상 및 트레저리 분배** — VCTBlock 이전에는 기존 ECCPoW 보상 정책을 유지하고, VCTBlock 이후에는 20 WL 블록 보상과 20% 트레저리 분배를 적용한다. 트레저리 주소와 분배 비율은 합의 규칙의 일부로 고정된다.

7. **하드포크 경계와 검증 경로** — 클라이언트는 VCTBlock 경계에서 레거시 ECCPoW 규칙과 VCT 규칙을 명확히 분기해야 한다. P2P 블록 import 경로와 로컬 채굴자가 생성한 블록을 체인에 쓰는 경로 모두에서 $S_0$ 자격 검증을 수행해야 하며, 부모 상태를 로드할 수 없는 경우 검증을 건너뛰지 않고 블록을 거부해야 한다.

8. **고정 sortition 임계값** — Rokis v1.0 클라이언트는 `P_base = 0xA0`을 합의 상수로 사용한다. 동적 `P_base` 조정, discrete level, cooldown, timeout window 기반 조정은 본 활성화 범위에 포함하지 않으며 후속 WIP에서 다룬다.

### Rokis/Daejeon Testnet Parameters

Rokis 하드포크 실험은 Daejeon 테스트넷에서 수행한다. Daejeon은 genesis부터 VCT를 적용하는 네트워크가 아니라, Seoul/ECCPoW 구간을 먼저 실행한 뒤 지정된 높이에서 VCT로 전환하는 하드포크 실험망이다.

참조 구현의 현재 실험 파라미터는 다음과 같다.
...
P_base (SortitionBase): 0xA0 (62.5%, fixed in Rokis v1.0)
TimeoutStart: 15 seconds
TimeoutEnd: 60 seconds
future timestamp tolerance F: 15 seconds
```

Genesis 설정에는 `vctBlock`뿐 아니라 `vct` 설정 객체도 포함되어야 한다. `vct` 설정이 없으면 클라이언트가 VCT 엔진을 생성하지 않고 기존 ECCPoW 엔진으로 폴백할 수 있으므로, Rokis 하드포크 실험에서는 최소한 다음과 같은 설정이 필요하다.
...
- [ ] `beta[0] < P_base` 기본 자격 임계값 판정 테스트 (경계값 포함)
...
- [ ] 잘못된 VRF 증명, 잘못된 채굴 서명, ECCPoW 실패 블록이 각각 거부되는 통합 테스트
- [x] 점진적 타임아웃: $\Delta t < t_s$일 때 `P(Δt) = P_base`임을 검증하는 단위 테스트
- [x] 점진적 타임아웃: $\Delta t = t_e$일 때 `P(Δt) = 256`임을 검증하는 단위 테스트
- [ ] 점진적 타임아웃: $t_\text{submit}(\beta_i[0])$ 이전의 타임스탬프를 사용한 블록이 검증자에 의해 거부되는 테스트
- [x] 점진적 타임아웃: $\Delta t \geq t_e$인 블록에서 임의의 첫 바이트 값이 모두 자격을 얻는 테스트
- [x] `MinEligibleBalanceAt`이 `S0ForkBlock` 전후의 $S_0$ 값을 올바르게 반환하는 단위 테스트
- [x] pre-VCT `legacySealHash`가 기존 ECCPoW `SealHash`와 일치하는지 검증하는 단위 테스트
- [ ] VCT 보상 20 WL, 20% 트레저리 분배, pre-VCT 레거시 보상 분기를 검증하는 state transition 테스트
- [ ] Daejeon/Rokis 2노드 하드포크 통합 테스트: block 0..99 ECCPoW 호환, block 100+ VCT 규칙 적용
- [ ] `P_base = 0xA0` 고정 임계값이 Rokis v1.0에서 변경되지 않는지 확인하는 단위 테스트
