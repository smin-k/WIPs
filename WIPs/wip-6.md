...
**이유.** $\ell = 1$은 구현 단순성을 최대화하고 기존 ECCPoW 검증 흐름과의 통합이 용이하다. 파라미터 $\ell$을 프로토콜 수준에서 명시함으로써, 향후 WIP에서 더 깊은 룩백이 필요하다고 판단될 경우 하드 포크로 조정할 수 있는 공간을 남겨둔다.
...
**WIP-6 선택.** 계정의 secp256k1 키 쌍 $(sk_i, PK_i)$를 ECVRF 입력 키로 그대로 사용한다. 블록 헤더에 $PK_i$를 포함하며, 검증자는 $\mathrm{Address}(PK_i) = a_i$를 확인하여 VRF 공개 키와 coinbase 주소의 일관성을 검증한다. 별도 VRF 키 등록 절차가 없으며, 계정 식별과 VRF 자격 증명이 통합된다. secp256k1 기반 ECVRF 구현은 aergoio/secp256k1-vrf [10] 및 vechain/go-ecvrf [11]를 참조한다.

**이유.** 계정 키 재사용은 키 관리 복잡성을 최소화하고 기존 Ethereum 계정 인프라와의 통합을 단순화한다. $a_i = \mathrm{Address}(PK_i)$ 관계를 통해 별도 등록 없이 블록 헤더에서 직접 검증할 수 있다.
